...
public class Cptest implements Runnable {
	...
	public void run() {
		...
		try {
			requestSocket = new Socket("172.16.0.177", 554);

			System.out.println("Connected to 172.16.0.177 in port 554");

			out = requestSocket.getOutputStream();

			out.write(("DESCRIBE rtsp://<ip>:<port>/unicast/c<channel number>/s<stream type>/live RTSP/1.0\r\n" + "CSeq: " + nextSeq() + "\r\n" + "User-Agent: Axis AMC\r\n" + "Accept: application/sdp\r\n" + "\r\n").getBytes());

			out.flush();

			parseDescription();

			out.write(("SETUP rtsp://<ip>:<port>/unicast/c<channel number>/s<stream type>/live RTSP/1.0\r\n" + "CSeq: " + nextSeq() + "\r\n" + "User-Agent: Axis AMC\r\n" + "Blocksize : 86000\r\n" + "Transport: RTP/AVP/TCP;unicast\r\n\r\n").getBytes());
			...
			out.write(("PLAY rtsp://<ip>:<port>/unicast/c<channel number>/s<stream type>/live RTSP/1.0\r\n" + "CSeq: " + nextSeq() + "\r\n" + "User-Agent: Axis AMC\r\n" + "Session: " + sessionIdentifier + " \r\n\r\n").getBytes());
			...
			out.write(("TEARDOWN rtsp://<ip>:<port>/unicast/c<channel number>/s<stream type>/live RTSP/1.0\r\n" + "CSeq: " + nextSeq() + "\r\n" + "User-Agent: Axis AMC\r\n" + "Session: " + sessionIdentifier + " \r\n\r\n").getBytes());
